...
export class ScheduleBoardComponent implements OnInit {
  ...
  public hcpTypeList: Array<string> = [
    'ACE Prioritization',
    'Test Prioritization',
  ];
  ...
  filterHCPList() {
    ...
    //hcp filter
    list = this.filterHCPItem(
      'hcpType',
      this.hcpType,
      prodFiltered ? list : this.hcpListAll
    );
    this.hcpList = list;
  }
  filterHCPItem(type: string, typeValue: string, array: Array<any>) {
    return array.filter(
      (item: { [x: string]: string }) => item[type] === typeValue
    );
  }
  changeHcpType() {
    this.filterHCPList();
  ...
  getCurrentPeriod() {
    if (this.globalStatusService.isMonthOrWeek) {
      ...
      //month
      this.currentPeriod =
        this.globalStatusService.currentPeriod.toLocaleString('en-us', {
          month: 'short',
        }) +
        ' ' +
